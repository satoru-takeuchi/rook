...
func getTolerations(clientset kubernetes.Interface) []corev1.Toleration {
	...
	if err != nil {
		// tolerationRaw is empty
		logger.Warningf("toleration will not be applied. %v", err)
		return tolerations
	}
	tolerations, err = k8sutil.YamlToTolerations(tolerationsRaw)
	if err != nil {
		logger.Warningf("failed to parse %q toleration will not be applied. %v", tolerationsRaw, err)
		return tolerations
	}
	return tolerations
}
...
func getNodeAffinity(clientset kubernetes.Interface) *corev1.NodeAffinity {
	...
	if nodeAffinity != "" {
		...
		if err != nil {
			logger.Warningf("failed to parse %q node affinity will not be applied. %v", nodeAffinity, err)
		}
	}
	return v1NodeAffinity
}
