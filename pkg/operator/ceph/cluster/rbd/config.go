...
func (r *ReconcileCephRBDMirror) createBootstrapPeerConfigAndKey(peerSecret string, tokenBase64 string, ownerInfo *client.OwnerInfo) error {
	...
	peerSiteUUID := r.peers[peerSecret].info.Peers[0].UUID
	cm, err := generatePeerCephConfigFileConfigMap(peerSiteUUID, cephPeerConfig, r.clusterInfo.Namespace, ownerInfo)
	if err != nil {
		return errors.Wrapf(err, "failed to generate kubernetes configmap for bootstrap peer config %q", peerSiteUUID)
	}
	...
	// Put it in a Secret
	s, err := generatePeerKeyringSecret(peerSiteUUID, r.clusterInfo.Namespace, cephPeerKey, ownerInfo)
	if err != nil {
		return errors.Wrapf(err, "failed to generate Kubernetes secret for bootstrap peer key %q", peerSiteUUID)
	}
...
func generatePeerCephConfigFileConfigMap(peerSiteUUID, peerCephConfig string, namespace string, ownerInfo *client.OwnerInfo) (*v1.ConfigMap, error) {
	cm := &v1.ConfigMap{
...
func generatePeerKeyringSecret(peerSiteUUID, peerCephKey string, namespace string, ownerInfo *client.OwnerInfo) (*v1.Secret, error) {
	s := &v1.Secret{
		ObjectMeta: metav1.ObjectMeta{
